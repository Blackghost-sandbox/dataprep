...
type ModuleId = "spark" | "sql" | "dbt" | "cloud" | "system";
const moduleContent = {spark:sparkLessons,sql:sqlLessons,dbt:dbtLessons,cloud:cloudLessons,system:systemDesignLessons};
const moduleLabels: Record<ModuleId,string> = {spark:"Apache Spark",sql:"SQL Fundamentals",dbt:"dbt",cloud:"Cloud Platforms",system:"System Design"};
const moduleRoadmapIndex: Record<ModuleId,number> = {sql:0,spark:3,dbt:6,cloud:7,system:8};
const enabledModules = new Map<number,ModuleId>([[0,"sql"],[3,"spark"],[6,"dbt"],[7,"cloud"],[8,"system"]]);
...
export function Sidebar({ collapsed, setCollapsed, onLesson, currentLesson, completed, module, onModule }: { collapsed: boolean; setCollapsed: (value: boolean) => void; onLesson: (lesson: string) => void; currentLesson: number; completed: number[]; module:ModuleId; onModule:(module:ModuleId)=>void }) {
  ...
  return <motion.aside animate={{ width: collapsed ? 88 : 280 }} transition={{ duration: .25 }} className="dataprep-sidebar fixed inset-y-0 left-0 z-30 flex flex-col overflow-hidden bg-[#081a36] text-white shadow-2xl">
    ...
    {!collapsed && <ProgressCard count={completed.length} total={lessons.length} name={module==="sql"?"SQL":module==="dbt"?"dbt":module==="cloud"?"Cloud":module==="system"?"System Design":"Spark"}/>}
...
export function DataPrepApp() {
  ...
  const [completion,setCompletion]=useState<Record<ModuleId,number[]>>({spark:[],sql:[],dbt:[],cloud:[],system:[]});
  const completed=completion[module];
  useEffect(() => {
    const restored:Record<ModuleId,number[]>={spark:[],sql:[],dbt:[],cloud:[],system:[]};
    for(const id of ["spark","sql","dbt","cloud","system"] as const) {
      try {
  ...
  return <div className={cn((module==="sql" || (module==="spark" && currentLesson === 1)) && "rdd-page", module==="sql" && "sql-module-page", "min-h-screen text-[#0f172a] transition-colors",dark ? "bg-[#e8ebf4]" : "bg-[#f5f7fb]")}>
    ...
    <motion.div animate={{marginLeft:collapsed ? 88 : 280}} transition={{duration:.25}} className="dataprep-main min-h-screen">
      ...
      <main className="mx-auto max-w-[1536px] space-y-4 p-5 xl:p-6">
        <div className="sql-module-picker"><label>Module <select value={module} onChange={e=>switchModule(e.target.value as ModuleId)}><option value="spark">Apache Spark</option><option value="sql">SQL Fundamentals</option><option value="dbt">dbt</option><option value="cloud">Cloud Platforms</option><option value="system">System Design</option></select></label><label>Lesson <select value={currentLesson} onChange={e=>openLesson(Number(e.target.value))}>{lessons.map((lesson,i)=><option key={lesson} value={i}>{i+1}. {lesson}</option>)}</select></label></div>
        ...
        <div className="lesson-layout grid grid-cols-[minmax(0,1fr)_320px] gap-4">
          <div className="lesson-content min-w-0 space-y-4">
            <motion.div key={module+"-"+currentLesson+"-"+active} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} className="space-y-4">
              {module==="sql" ? <SparkLessonPanel key={"sql-"+sqlLessons[currentLesson].id} module="sql" lesson={sqlLessons[currentLesson]} active={active} onTab={setActive} onLesson={id=>{const index=sqlLessons.findIndex(lesson=>lesson.id===id);if(index>=0)openLesson(index);}}/> : module==="dbt" ? <SparkLessonPanel key={"dbt-"+dbtLessons[currentLesson].id} module="dbt" lesson={dbtLessons[currentLesson]} active={active}/> : module==="cloud" ? <SparkLessonPanel key={"cloud-"+cloudLessons[currentLesson].id} module="cloud" lesson={cloudLessons[currentLesson]} active={active}/> : module==="system" ? <SparkLessonPanel key={"system-"+systemDesignLessons[currentLesson].id} module="system" lesson={systemDesignLessons[currentLesson]} active={active}/> : currentLesson === 1 && active !== "Notes" ? active === "Concept" ? <ConceptPanel/> : <InteractivePanel active={active}/> : <SparkLessonPanel key={sparkLessons[currentLesson].id} lesson={sparkLessons[currentLesson]} active={active}/>}
            </motion.div>
