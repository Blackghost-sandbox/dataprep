...
const lessons = sparkLessons.map(lesson => lesson.title);
...
export function ProgressCard({ count }: { count: number }) {
  return <button onClick={() => announce(`${count} of 10 Spark lessons marked complete`)} className="mx-4 block w-[calc(100%-2rem)] rounded-2xl border border-white/10 bg-white/[.055] p-4 text-left hover:bg-white/[.08]">
    <span className="mb-2 flex items-center justify-between text-sm"><span className="font-semibold">Spark Progress</span><span className="font-bold text-emerald-400">{count * 10}%</span></span>
    <Progress value={count * 10} className="h-2 bg-white/10 [&_[data-slot=progress-indicator]]:bg-gradient-to-r [&_[data-slot=progress-indicator]]:from-emerald-400 [&_[data-slot=progress-indicator]]:to-teal-300"/>
    <span className="mt-2 block text-xs text-slate-400">Keep building your momentum</span>
  </button>;
}

export function Sidebar({ collapsed, setCollapsed, onLesson, currentLesson, completed }: { collapsed: boolean; setCollapsed: (value: boolean) => void; onLesson: (lesson: string) => void; currentLesson: number; completed: number[] }) {
  const [sparkOpen, setSparkOpen] = useState(true);
  return <motion.aside animate={{ width: collapsed ? 88 : 280 }} transition={{ duration: .25 }} className="fixed inset-y-0 left-0 z-30 flex flex-col overflow-hidden bg-[#081a36] text-white shadow-2xl">
    ...
    {!collapsed && <ProgressCard count={completed.length}/>}
    <nav className="mt-4 min-h-0 flex-1 overflow-y-auto px-3 scrollbar-none">
      ...
      <div className="space-y-1.5">{modules.map((item, i) => <div key={item}>
        <button onClick={() => i === 3 ? setSparkOpen(!sparkOpen) : announce(`${item} is outside this Spark module`)} aria-expanded={i === 3 ? sparkOpen : undefined} className={cn("flex w-full items-center rounded-xl text-left transition-colors", collapsed ? "justify-center p-2" : "gap-3 px-3 py-2.5", i === 3 ? "bg-[#2450a2] shadow-lg shadow-blue-950/30" : "hover:bg-white/[.06]")}>
          <span className={cn("grid size-8 shrink-0 place-items-center rounded-full border text-sm font-semibold", i === 3 ? "border-blue-300 bg-blue-500 text-white ring-4 ring-blue-400/15" : "border-white/10 bg-[#294363] text-slate-200")}>{i + 1}</span>
          {!collapsed && <span className="flex-1 text-sm leading-tight">{item}</span>}{!collapsed && i === 3 && <ChevronDown size={16}/>} 
        </button>
        <AnimatePresence>{!collapsed && i === 3 && sparkOpen && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="ml-7 overflow-hidden border-l border-blue-300/20 py-1 pl-4">
...
export function TopNavbar({ dark, setDark, setTab }: { dark: boolean; setDark: (value: boolean) => void; setTab: (value: string) => void }) {
  ...
  return <header className="sticky top-0 z-20 flex h-[72px] items-center gap-7 border-b border-[#e7ecf3] bg-white/90 px-6 backdrop-blur-xl">
...
export function LessonTabs({ active, setActive }: { active: string; setActive: (value: string) => void }) {
  return <Tabs value={active} onValueChange={setActive} className="rounded-2xl border border-[#e7ecf3] bg-white px-4 shadow-[0_8px_28px_rgba(34,48,73,.05)]"><TabsList variant="line" className="flex h-[58px] w-full justify-between gap-0">{tabs.map(([label, Icon]) => <TabsTrigger key={label} value={label} className="h-full px-3 text-[13px] font-semibold after:bottom-0 after:h-[3px] after:rounded-full after:bg-[#6d5df6] data-[state=active]:text-[#5b4deb]"><Icon size={17}/>{label}</TabsTrigger>)}</TabsList></Tabs>;
}
...
function RightPanel({ currentLesson, onLesson, completed }: { currentLesson: number; onLesson: (lesson: string) => void; completed: number[] }) {
  return <aside className="space-y-4"><Surface><details open className="lesson-playlist"><summary className="mb-3 flex cursor-pointer items-center justify-between"><span className="font-bold">Lesson Progress <ChevronDown size={16} className="inline-block text-slate-400" aria-hidden="true"/></span><span className="text-xs font-semibold text-[#6d5df6]">{completed.length} / 10 done</span></summary><Progress value={completed.length * 10} className="mb-5 h-2 bg-slate-100 [&_[data-slot=progress-indicator]]:bg-gradient-to-r [&_[data-slot=progress-indicator]]:from-[#6d5df6] [&_[data-slot=progress-indicator]]:to-[#9b8fff]"/><div className="space-y-1">{lessons.map((lesson, i) => <button onClick={() => onLesson(lesson)} aria-current={i === currentLesson ? "step" : undefined} key={lesson} className={cn("flex w-full items-center gap-2.5 rounded-xl px-2.5 py-2 text-left text-[13px] hover:bg-slate-50", i === currentLesson ? "bg-[#eeeaff] font-semibold text-[#5b4deb]" : "text-slate-600")}>{completed.includes(i) ? <CheckCircle2 size={18} className="text-emerald-500"/> : i === currentLesson ? <Play size={18} className="fill-[#6d5df6] text-[#6d5df6]"/> : <Circle size={18} className="text-slate-300"/>}<span>{i + 1}. {lesson}</span></button>)}</div></details></Surface><button onClick={() => announce("Quick note pinned")} className="w-full rotate-[-.6deg] rounded-[18px] border border-amber-200 bg-[#fff9d9] p-5 text-left shadow-[0_8px_24px_rgba(120,87,20,.08)]"><div className="mb-2 flex items-center gap-2 font-bold"><NotebookPen size={19} className="text-amber-600"/>Quick Notes</div><p className="text-sm leading-6 text-amber-950/75">{currentLesson === 1 ? "DataFrames provide a schema and an optimized query engine. Choose RDDs when their extra control fits your task." : sparkLessons[currentLesson].concepts[0]?.[1]}</p></button><ResourceList/><QuoteCard/></aside>;
}

function Hero({ onPrevious, onNext, currentLesson, compact = false }: { onPrevious: () => void; onNext: () => void; currentLesson: number; compact?: boolean }) {
  return <div className={`${compact ? "practice-hero " : ""}relative overflow-hidden rounded-[22px] border border-[#e7ecf3] bg-gradient-to-r from-white via-white to-[#eef2ff] p-6 shadow-[0_8px_28px_rgba(34,48,73,.04)]`}><Mountain className="absolute -bottom-7 right-10 size-44 text-[#6d5df6]/[.07]"/><div className="relative flex items-center justify-between"><div><div className="mb-4 flex items-center gap-2 text-sm text-slate-500"><button onClick={() => announce("Apache Spark roadmap opened")} className="hover:text-[#6d5df6]">Apache Spark</button><ChevronRight size={14}/><span className="font-medium text-slate-700">{lessons[currentLesson]}</span></div><div className="flex items-center gap-4"><div className="grid size-16 place-items-center rounded-2xl bg-[#eeeaff] text-[#6d5df6]"><Zap size={31}/></div><div><h1 className="text-[40px] font-bold leading-tight tracking-[-.035em]">{lessons[currentLesson]}</h1><p className="mt-1 max-w-2xl text-[15px] leading-6 text-slate-600">{sparkLessons[currentLesson].description}</p></div></div></div><div className="flex flex-col items-end gap-4"><button onClick={() => announce(currentLesson === 0 ? "Difficulty: Beginner" : "Difficulty: Intermediate")} className="rounded-full bg-amber-100 px-3 py-1.5 text-xs font-bold text-amber-700">{currentLesson === 0 ? "Beginner" : "Intermediate"}</button><div className="flex gap-2"><button onClick={onPrevious} disabled={currentLesson === 0} aria-label="Previous lesson" className="grid size-10 place-items-center rounded-xl border bg-white text-slate-600 hover:bg-slate-50"><ChevronLeft size={18}/></button><button onClick={onNext} disabled={currentLesson === lessons.length-1} className="flex items-center gap-2 rounded-xl bg-[#6d5df6] px-5 py-2.5 text-sm font-bold text-white shadow-lg shadow-[#6d5df6]/20 hover:bg-[#5b4deb]">Next <ChevronRight size={17}/></button></div></div></div><div className="relative mt-5 flex gap-2">{[[Clock3, `${sparkLessons[currentLesson].minutes} min`], [GraduationCap, `Lesson ${currentLesson + 1}/10`], [Users, "8,342 learners"]].map(([Icon, label]) => <button onClick={() => announce(String(label))} key={String(label)} className="flex items-center gap-2 rounded-full border border-slate-200 bg-white/80 px-3 py-1.5 text-xs font-medium text-slate-600 hover:border-[#6d5df6]/30"><Icon size={14}/>{String(label)}</button>)}</div></div>;
}

function ConceptPanel() { return <><Surface><SectionTitle icon={BookOpen} title="What are RDD and DataFrame?"/><p className="mb-5 text-[15px] leading-6 text-slate-600"><GlossaryText>Apache Spark gives you two powerful ways to work with distributed data. RDDs expose the engine’s low-level building blocks, while DataFrames add structure and an optimizer that plans efficient execution.</GlossaryText></p><FeatureComparison/></Surface><Surface><SectionTitle icon={Network} title="How Spark processes each model"/><ArchitectureDiagram/></Surface><Surface><SectionTitle icon={Code2} title="Code Example"/><CodeBlock/></Surface><button onClick={() => announce("Takeaway saved to your notes")} className="flex w-full items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-left text-sm leading-6 text-amber-950 hover:shadow-md"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-amber-100 text-amber-600"><Lightbulb size={20}/></span><span><span className="block font-bold">Key Takeaway</span>Use DataFrames for most analytics workloads because Catalyst can optimize the execution plan. Reach for RDDs when you need fine-grained control.</span></button></>; }
...
export function DataPrepApp() {
  ...
  useEffect(() => {
    try {
      const value = JSON.parse(localStorage.getItem("dataprep.spark.completed.v1") || "[]");
      if (Array.isArray(value)) setCompleted([...new Set(value.filter((n: unknown): n is number => typeof n === "number" && Number.isInteger(n) && n >= 0 && n < lessons.length))]);
    } catch { toast.error("Could not restore lesson progress on this device."); }
  }, []);
  const openLesson = (index: number) => {
    setCurrentLesson(index);
    setActive(index === 6 ? "Hands-on" : index === 7 ? "Interview Qs" : index === 8 ? "Quiz" : "Concept");
  ...
  const toggleComplete = () => {
    ...
    try { localStorage.setItem("dataprep.spark.completed.v1",JSON.stringify(updated)); }
    catch { toast.error("Progress updated for this visit but could not be saved."); }
  };
  return <div className={cn("min-h-screen text-[#0f172a] transition-colors",dark ? "bg-[#e8ebf4]" : "bg-[#f5f7fb]")}>
    <Toaster position="bottom-right" richColors/>
    <Sidebar collapsed={collapsed} setCollapsed={setCollapsed} onLesson={selectLesson} currentLesson={currentLesson} completed={completed}/>
    <motion.div animate={{marginLeft:collapsed ? 88 : 280}} transition={{duration:.25}} className="min-h-screen">
      <TopNavbar dark={dark} setDark={setDark} setTab={setActive}/>
      <main className="mx-auto max-w-[1536px] space-y-4 p-5 xl:p-6">
        <Hero compact={["Interview Qs","Hands-on","Quiz","Common Mistakes"].includes(active)} currentLesson={currentLesson} onPrevious={()=>moveLesson(-1)} onNext={()=>moveLesson(1)}/>
        <LessonTabs active={active} setActive={setActive}/>
        <div className="grid grid-cols-[minmax(0,1fr)_320px] gap-4">
          <div className="min-w-0 space-y-4">
            <motion.div key={currentLesson+"-"+active} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} className="space-y-4">
              {currentLesson === 1 && active !== "Notes" ? active === "Concept" ? <ConceptPanel/> : <InteractivePanel active={active}/> : <SparkLessonPanel key={sparkLessons[currentLesson].id} lesson={sparkLessons[currentLesson]} active={active}/>}
            </motion.div>
            <section className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-5">
              ...
              <button disabled={currentLesson===9} onClick={()=>moveLesson(1)} className="text-sm font-semibold text-[#6d5df6] disabled:opacity-40">{currentLesson===9 ? "End of Spark module" : "Next: "+lessons[currentLesson+1]+" →"}</button>
            </section>
          </div>
          <RightPanel currentLesson={currentLesson} onLesson={selectLesson} completed={completed}/>
        </div>
      </main>
    </motion.div>
  </div>;
