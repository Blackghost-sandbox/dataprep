...
type ModuleId = "spark" | "sql" | "dbt" | "cloud";
const moduleContent = {spark:sparkLessons,sql:sqlLessons,dbt:dbtLessons,cloud:cloudLessons};
const moduleLabels: Record<ModuleId,string> = {spark:"Apache Spark",sql:"SQL Fundamentals",dbt:"dbt",cloud:"Cloud Platforms"};
const moduleRoadmapIndex: Record<ModuleId,number> = {sql:0,spark:3,dbt:6,cloud:7};
const enabledModules = new Map<number,ModuleId>([[0,"sql"],[3,"spark"],[6,"dbt"],[7,"cloud"]]);
...
export function Sidebar({ collapsed, setCollapsed, onLesson, currentLesson, completed, module, onModule }: { collapsed: boolean; setCollapsed: (value: boolean) => void; onLesson: (lesson: string) => void; currentLesson: number; completed: number[]; module:ModuleId; onModule:(module:ModuleId)=>void }) {
  ...
  return <motion.aside animate={{ width: collapsed ? 88 : 280 }} transition={{ duration: .25 }} className="dataprep-sidebar fixed inset-y-0 left-0 z-30 flex flex-col overflow-hidden bg-[#081a36] text-white shadow-2xl">
    ...
    {!collapsed && <ProgressCard count={completed.length} total={lessons.length} name={module==="sql"?"SQL":module==="dbt"?"dbt":module==="cloud"?"Cloud":"Spark"}/>}
...
export function DataPrepApp() {
  ...
  const [completion,setCompletion]=useState<Record<ModuleId,number[]>>({spark:[],sql:[],dbt:[],cloud:[]});
  const completed=completion[module];
  useEffect(() => {
    const restored:Record<ModuleId,number[]>={spark:[],sql:[],dbt:[],cloud:[]};
    for(const id of ["spark","sql","dbt","cloud"] as const) {
      try {
